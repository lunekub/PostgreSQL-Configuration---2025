...
### บันทึกผลการทดลอง
```
- docker exec postgres-config free → แสดงหน่วยความจำ (RAM) ที่ใช้และว่างใน container
- docker exec postgres-config df → แสดงพื้นที่ดิสก์ที่ใช้และเหลือใน container

- -h → แสดงผลในรูปแบบอ่านง่าย (human-readable) เช่น MB, GB

- docker exec postgres-config nproc → แสดงจำนวน CPU core ที่ container ใช้งานได้
```
...
- ค่า shared_buffers = 16384 × 8KB = 131,072 KB (≈ 128 MB)
3. ค่า  pending_restart ในผลการทดลองมีค่าเป็นอย่างไร และมีความหมายอย่างไร
- ค่า pending_restart = f หมายความว่า ไม่ต้องรีสตาร์ท PostgreSQL เพื่อให้ค่าการตั้งค่านี้มีผล
```
-- คำนวณและตั้งค่าใหม่
...
- ใช้สำหรับวิเคราะห์และแสดง แผนการทำงานจริงของคำสั่ง SQL พร้อมเวลาประมวลผลจริง (ANALYZE) และการใช้หน่วยความจำแคช (BUFFERS) เช่น shared hit, read, write
2. รูปผลการรัน
![alt text](image-10.png)
3. อธิบายผลลัพธ์ที่ได้
-PostgreSQL ใช้ Parallel Query โดยมี Workers Planned: 2 และ Workers Launched: 2
  ข้อมูลถูกอ่านแบบ Parallel Seq Scan จากตาราง large_table
  ใช้วิธี Top-N Heapsort เพื่อเรียงข้อมูลตาม Sort Key: data
  Buffers: shared hit=5135 → ดึงข้อมูลจากแคชหน่วยความจำ ไม่ต้องอ่านจากดิสก์
  Execution Time: 163.826 ms → เวลาที่ใช้รันคำสั่งจริงทั้งหมด
  สรุป: Query นี้อ่านข้อมูลจากแคชเป็นส่วนใหญ่ ทำงานแบบขนาน (parallel) และใช้เวลารวมประมาณ 163.8 ms ✅
...
- คำสั่งใช้ GroupAggregate เพื่อจัดกลุ่มข้อมูลตามคอลัมน์ number
ใช้ Filter (count(*) > 1) กรองเฉพาะค่าที่ซ้ำ
ใช้ Index Only Scan ผ่านดัชนี idx_large_table_number
ไม่ต้องอ่านข้อมูลจากตารางจริง (Heap Fetches: 0) เพราะค่าที่ต้องการอยู่ในดัชนี
ใช้เวลาเพียง 0.322 ms → เร็วมากเพราะใช้ index อย่างมีประสิทธิภาพ
3. การสแกนเป็นแบบใด เกิดจากเหตุผลใด
- 👉 Index Only Scan
เหตุผล:
เพราะคอลัมน์ที่ query ใช้ (number) ถูกจัดทำดัชนีไว้ และ PostgreSQL สามารถดึงค่าที่ต้องการทั้งหมดได้จาก index โดยไม่ต้องอ่านข้อมูลจาก heap (ตารางจริง) จึงประมวลผลได้รวดเร็วมาก
...
2. อธิบายผลลัพธ์ที่ได้
VACUUM: ลบข้อมูลที่หมดอายุและคืนพื้นที่ในตาราง large_table
ANALYZE: เก็บสถิติของตารางเพื่อให้ตัว optimizer ใช้ปรับแผน query
สรุป: ไม่มีแถวถูกลบ (0 dead rows), ตารางสะอาด และมีการอัปเดตสถิติเรียบร้อย ✅
...
2. อธิบายผลลัพธ์ที่ได้
- PostgreSQL อ่านข้อมูลทั้งหมดจาก cache (shared buffers) ไม่ต้องอ่านจากดิสก์เลย → ประสิทธิภาพสูงสุด
...
2. อธิบายผลลัพธ์ที่ได้
- เกือบทุกการอ่านข้อมูลมาจาก cache (shared buffers) ไม่ต้องอ่านจากดิสก์ → ระบบทำงานมีประสิทธิภาพสูงมาก
...
2. อธิบายผลลัพธ์ที่ได้
- ข้อมูลทั้งหมดถูกอ่านจาก cache (shared buffers) แล้ว → ระบบทำงานได้เร็วมาก เพราะไม่ต้องเข้าถึงดิสก์เลย
...
autovacuum → เปิด/ปิดการทำงานของ autovacuum (ค่า on = เปิดอยู่ ✅)
autovacuum_naptime (60s) → ระยะเวลาระหว่างการตรวจสอบรอบถัดไปของ autovacuum
autovacuum_max_workers (3) → จำนวน worker สูงสุดที่สามารถทำงานพร้อมกันได้
autovacuum_vacuum_scale_factor (0.2) → สัดส่วนของแถวที่เปลี่ยนแปลงก่อนจะเริ่ม vacuum (20%)
autovacuum_analyze_scale_factor (0.1) → สัดส่วนของแถวที่เปลี่ยนแปลงก่อนจะเริ่ม analyze (10%)
autovacuum_vacuum_threshold (50) และ autovacuum_analyze_threshold (50) → จำนวนแถวเปลี่ยนแปลงขั้นต่ำที่จะกระตุ้นการ vacuum/analyze
autovacuum_vacuum_cost_delay (2 ms) → หน่วงเวลาแต่ละรอบ vacuum เพื่อไม่ให้กิน CPU มากเกินไป
log_autovacuum_min_duration (600000 ms) → จะบันทึก log ถ้า autovacuum ใช้เวลามากกว่า 10 นาที
...
2. อธิบายผลลัพธ์ที่ได้
- ไม่มีข้อมูลในตาราง performance_results ที่ตรงกับคำสั่ง SELECT
...
SELECT (JOIN + WHERE) → เร็วที่สุด ใช้เวลาเฉลี่ยเพียง 0.033 ms เพราะอ่านจากแคช
INSERT → ใช้เวลาเฉลี่ย 0.118 ms ถือว่าเร็วมาก
UPDATE → ช้ากว่าเล็กน้อย (84.121 ms) เพราะต้องเขียนข้อมูลทับและจัดการ index
DELETE (soft) → ช้าที่สุด (94.874 ms) เนื่องจากต้องอัปเดต flag deleted_at และจัดการ consistency
...
## คำถามท้ายการทดลอง
1. Shared Memory:
- shared_buffers, wal_buffers → ใช้เก็บข้อมูลใน RAM ร่วมกัน
- ตั้งค่า shared_buffers ≈ 25–40% ของ RAM
2. Work / Maintenance Work Mem:
- work_mem ใช้ตอน query (sort, join)
- maintenance_work_mem ใช้ตอน VACUUM, CREATE INDEX
ตั้งค่าให้พอดีกับจำนวน connection
3. ตัวอย่าง (RAM 16GB, conn 200):
- shared_buffers = 4GB
- work_mem = 32MB
- maintenance_work_mem = 2GB
4. postgresql.conf / auto.conf:
- auto.conf ถูกสร้างจาก ALTER SYSTEM และ override ค่าจาก postgresql.conf
5. Buffer Hit Ratio:
- สัดส่วนการอ่านจาก cache ใน RAM (>95% ดี)
6. ตัวอย่างเครื่อง 16GB:
- shared_buffers=4GB, work_mem=32MB, maintenance_work_mem=2GB, effective_cache_size=10GB
7. การสแกนข้อมูล:
- Seq Scan: อ่านทั้งตาราง
- Index Scan: ใช้ index
- Bitmap Scan: รวม index หลายอัน
- Index Only Scan: อ่านเฉพาะจาก index (เร็วสุด)